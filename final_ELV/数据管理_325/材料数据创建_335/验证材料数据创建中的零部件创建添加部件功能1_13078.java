 ...
 * 所属模块: 材料数据创建(#335)
 * 用例名称: 验证材料数据创建中的零部件创建添加部件功能1
 * 前置条件: 能正确进入材料数据管理界面 
 * 测试步骤->1. 点击序号为1的零部件右侧的创建按钮
 * 测试预期->1. 跳转至创建零部件页面
 * 测试步骤->2. 点击页面左上方的添加半成品按钮
 * 测试预期->2. 成功在零部件下方添加一半成品
 * 测试步骤->3. 再次点击左上方添加材料按钮
 * 测试预期->3. 跳转至选择材料按钮
 * 测试步骤->4. 点击检索按钮，任意选择一个材料，点击添加按钮
 * 测试预期->4. 页面提示添加失败：“同级节点类型必须保持一致”
 * 测试步骤->5. 点击页面左上方的添加半成品按钮
 * 测试预期->5. 成功在零部件下方添加一半成品
 * 测试步骤->6. 再次点击左上方添加材料按钮
 * 测试预期->6. 跳转至选择材料按钮
 * 测试步骤->7. 点击检索按钮，任意选择一个材料，点击添加按钮
 * 测试预期->7. 页面提示添加失败：“同级节点类型必须保持一致”
 * 
 */

public class 验证材料数据创建中的零部件创建添加部件功能1_13078 extends Selenium_Test_Base {
	
	public 验证材料数据创建中的零部件创建添加部件功能1_13078(){
		super();
	}
	
	public 验证材料数据创建中的零部件创建添加部件功能1_13078(WebDriver driver){
		super(driver);
	}
