 ...
 * 所属模块: MDS数据同步(#332)
 * 用例名称: 验证MDS数据同步下方功能栏按钮是否可以实现-含异常操作
 * 前置条件: 1.有CAMDS帐号，拥有MDS数据同步的菜单权限。
 *         2.能够正确进入MDS数据同步界面
 * 测试步骤->1. 点击MDS数据同步界面最下方的翻页功能栏中的下一页按钮
 * 测试预期->1. 页面信息跳转至下一页，页面其余内容正常显示。
 * 测试步骤->2. 点击MDS数据同步界面最下方的翻页功能栏中的最后一页按钮
 * 测试预期->2. 页面信息跳转至最后一页，页面其余内容正常显示。
 * 测试步骤->3. 点击MDS数据同步界面最下方的翻页功能栏中的填写框填写1，按回车键。
 * 测试预期->3. 页面信息跳转至第1页，页面其余内容正常显示。
 * 测试步骤->4. 点击MDS数据同步界面最下方的翻页功能栏中的填写框填写2000000，按回车键。
 * 测试预期->4. 页面信息跳转至最后一页，页面其余内容正常显示。
 * 
 */

public class 验证MDS数据同步下方功能栏按钮是否可以实现_含异常操作_3354 extends Selenium_Test_Base {
	
	public 验证MDS数据同步下方功能栏按钮是否可以实现_含异常操作_3354(){
		super();
	}
	
	public 验证MDS数据同步下方功能栏按钮是否可以实现_含异常操作_3354(WebDriver driver){
		super(driver);
	}
