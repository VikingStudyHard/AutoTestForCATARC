 ...
 * 所属模块: 审核信息(#334)
 * 用例名称: 验证查询MDS审核信息的审核按钮是否正常工作
 * 前置条件: 拥有MDS审核信息的权限
 * 测试步骤->1. 点击审核按钮
 * 测试预期->1. 出现提示信息：请选择一个MDS！
 * 测试步骤->2. 勾选任意两项MDS信息
 * 测试预期->2. 功能栏按钮部分消失，仅剩下审核按钮。
 * 测试步骤->3. 勾选一项MDS状态为内部发布，审核状态为未审批的MDS信息，点击审核按钮
 * 测试预期->3. 跳出新页面，页面标题为校验结果，页面包含待审核的各项信息。
 * 测试步骤->4. 勾选一项MDS状态为内部发布，审核状态为已审批的MDS信息，点击审核按钮
 * 测试预期->4. 出现提示信息：不可以审核该零部件测试5.4 0330已经审核！
 * 测试步骤->5. 勾选一项MDS状态为已发布，审核状态为审批中的MDS信息，点击审核按钮
 * 测试预期->5. 出现提示信息：不可以审核该零部件PAKC homogeneous adhesive tape已经审核！
 * 测试步骤->6. 勾选一项MDS状态为已提交，审核状态为未审批的MDS信息，点击审核按钮
 * 测试预期->6. 跳出新页面，页面标题为校验结果，页面包含待审核的各项信息。
 * 测试步骤->7. 勾选一项MDS状态为内部发布，审核状态为审批中的MDS信息，点击审核按钮
 * 测试预期->7. 出现提示信息：不可以审核该零部件测试5.4已经审核！
 * 
 */

public class 验证查询MDS审核信息的审核按钮是否正常工作_3460 extends Selenium_Test_Base {
	
	public 验证查询MDS审核信息的审核按钮是否正常工作_3460(){
		super();
	}
	
	public 验证查询MDS审核信息的审核按钮是否正常工作_3460(WebDriver driver){
		super(driver);
	}
