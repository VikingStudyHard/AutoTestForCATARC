 ...
 * 所属模块: 基础数据同步(#331)
 * 用例名称: 验证基础数据同步下方功能栏按钮是否可以实现-含异常操作
 * 前置条件: 1.有正确的KEY值
 *         2.系统部署时的CAMDS账户应能够正常登录CAMDS系统
 *         3.能够正确进入基础数据同步界面
 * 测试步骤->1. 点击基础数据同步界面最下方的翻页功能栏中的下一页按钮
 * 测试预期->1. 页面信息跳转至下一页，页面其余内容正常显示。
 * 测试步骤->2. 点击基础数据同步界面最下方的翻页功能栏中的最后一页按钮
 * 测试预期->2. 页面信息跳转至最后一页，页面其余内容正常显示。
 * 测试步骤->3. 点击基础数据同步界面最下方的翻页功能栏中的填写框填写1，按回车键。
 * 测试预期->3. 页面信息跳转至第1页，页面其余内容正常显示。
 * 测试步骤->4. 点击基础数据同步界面最下方的翻页功能栏中的填写框填写2000000，按回车键。
 * 测试预期->4. 页面信息跳转至最后一页，页面其余内容正常显示。
 * 
 */

public class 验证基础数据同步下方功能栏按钮是否可以实现_含异常操作_3325 extends Selenium_Test_Base {
	
	public 验证基础数据同步下方功能栏按钮是否可以实现_含异常操作_3325(){
		super();
	}
	
	public 验证基础数据同步下方功能栏按钮是否可以实现_含异常操作_3325(WebDriver driver){
		super(driver);
	}
