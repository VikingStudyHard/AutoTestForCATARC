 ...
 * 所属模块: 个人信息(#359)
 * 用例名称: 验证用户个人密码修改功能是否可以实现-不操作
 * 前置条件: 能正确进入ELV环境合规系统（标准版）个人信息界面
 * 测试步骤->1. 点击修改个人密码按钮
 * 测试预期->1. 跳出所需填写内容的新页面
 * 测试步骤->2. 在新页面上错误填写原始密码，新密码，再次输入新密码，点击确认。
 * 测试预期->2. 提示输入的旧密码不正确
 * 测试步骤->3. 关闭修改密码界面
 * 测试预期->3. 密码修改失败，现密码为修改操作前密码。
 * 
 */

public class 验证用户个人密码修改功能是否可以实现_不操作_1039 extends Selenium_Test_Base {
	
	public 验证用户个人密码修改功能是否可以实现_不操作_1039(){
		super();
	}
	
	public 验证用户个人密码修改功能是否可以实现_不操作_1039(WebDriver driver){
		super(driver);
	}
