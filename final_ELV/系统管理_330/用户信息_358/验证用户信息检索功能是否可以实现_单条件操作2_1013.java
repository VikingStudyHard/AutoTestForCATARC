 ...
 * 所属模块: 用户信息(#358)
 * 用例名称: 验证用户信息检索功能是否可以实现-单条件操作2
 * 前置条件: 能正确进入ELV环境合规系统（标准版）用户信息界面
 * 测试步骤->1. 在查询条件中的姓名中填写李德平，登录名中填写lideping，点击查询按钮
 * 测试预期->1. 页面查询到用户李德平
 * 
 */

public class 验证用户信息检索功能是否可以实现_单条件操作2_1013 extends Selenium_Test_Base {
	
	public 验证用户信息检索功能是否可以实现_单条件操作2_1013(){
		super();
	}
	
	public 验证用户信息检索功能是否可以实现_单条件操作2_1013(WebDriver driver){
		super(driver);
	}
	...
	public void detailedOperation(){
		...
		WebDriverConfiguration.operationInterval();
		getElement(ElementEnum_用户信息_358.登录名输入框).sendKeys("lideping");
		WebDriverConfiguration.operationInterval();
