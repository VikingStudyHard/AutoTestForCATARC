 ...
 * 所属模块: 用户信息(#358)
 * 用例名称: 验证用户信息新增功能是否可以实现-总流程
 * 前置条件: 能正确进入ELV环境合规系统（标准版）用户信息界面
 * 测试步骤->1. 点击新增按钮
 * 测试预期->1. 跳出新页面显示新增一用户所需要填写的各项内容。
 * 测试步骤->2. 必要填写项目不填写完整，点击保存按钮。
 * 测试预期->2. 出现提示信息：必要填写项目不能为空。
 * 测试步骤->3. 点击提示信息中的确定按钮并点击关闭。
 * 测试预期->3. 跳转回最初页面，无新增用户。
 * 
 */

public class 验证用户信息新增功能是否可以实现_总流程_1027 extends Selenium_Test_Base {
	
	public 验证用户信息新增功能是否可以实现_总流程_1027(){
		super();
	}
	
	public 验证用户信息新增功能是否可以实现_总流程_1027(WebDriver driver){
		super(driver);
	}
