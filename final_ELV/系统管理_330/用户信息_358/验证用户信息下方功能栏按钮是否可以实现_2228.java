 ...
 * 所属模块: 用户信息(#358)
 * 用例名称: 验证用户信息下方功能栏按钮是否可以实现
 * 前置条件: 能正确进入ELV环境合规系统（标准版）用户信息界面
 * 测试步骤->1. 点击用户信息界面最下方的翻页功能栏中的下一页按钮
 * 测试预期->1. 页面信息跳转至下一页，页面其余内容正常显示。
 * 测试步骤->2. 点击用户信息界面最下方的翻页功能栏中的最后一页按钮
 * 测试预期->2. 页面信息跳转至最后一页，页面其余内容正常显示。
 * 测试步骤->3. 点击用户信息界面最下方的翻页功能栏中的填写框填写1，按回车键。
 * 测试预期->3. 页面信息跳转至第1页，页面其余内容正常显示。
 * 测试步骤->4. 点击用户信息界面最下方的翻页功能栏中的填写框填写2000000，按回车键。
 * 测试预期->4. 页面信息跳转至最后一页，页面其余内容正常显示。
 * 
 */

public class 验证用户信息下方功能栏按钮是否可以实现_2228 extends Selenium_Test_Base {
	
	public 验证用户信息下方功能栏按钮是否可以实现_2228(){
		super();
	}
	
	public 验证用户信息下方功能栏按钮是否可以实现_2228(WebDriver driver){
		super(driver);
	}
	...
	public void detailedOperation(){
		...
		getElement(ElementEnum_用户信息_358.页码显示及输入框).clear();
		getElement(ElementEnum_用户信息_358.页码显示及输入框).sendKeys("1");
		getElement(ElementEnum_用户信息_358.页码显示及输入框).sendKeys(Keys.ENTER);
		for(int i= 0;i < 5;i++)
			WebDriverConfiguration.operationInterval();
		WebDriverConfiguration.operationInterval();
		getElement(ElementEnum_用户信息_358.页码显示及输入框).clear();
		getElement(ElementEnum_用户信息_358.页码显示及输入框).sendKeys("2000000");
		getElement(ElementEnum_用户信息_358.页码显示及输入框).sendKeys(Keys.ENTER);
		for(int i= 0;i < 5;i++)
			WebDriverConfiguration.operationInterval();
		WebDriverConfiguration.operationInterval();
