 ...
 * 所属模块: 日志信息(#360)
 * 用例名称: 验证日志信息检索功能是否可以实现-正常操作
 * 前置条件: 能正确进入ELV环境合规系统（标准版）日志信息界面
 * 测试步骤->1. 在检索条件中的开始时间选择今日日期，结束时间选择今日日期，点击检索。
 * 测试预期->1. 页面正确检索出今天所有的日志信息。
 * 
 */

public class 验证日志信息检索功能是否可以实现_正常操作_1044 extends Selenium_Test_Base {
	
	public 验证日志信息检索功能是否可以实现_正常操作_1044(){
		super();
	}
	
	public 验证日志信息检索功能是否可以实现_正常操作_1044(WebDriver driver){
		super(driver);
	}
