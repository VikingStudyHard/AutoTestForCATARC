 ...
 * 所属模块: 日志信息(#360)
 * 用例名称: 验证日志信息按照检索条件删除功能是否可以实现-正常处理
 * 前置条件: 能正确进入ELV环境合规系统（标准版）日志信息界面
 * 测试步骤->1. 在检索条件中的开始时间选择今日日期，结束时间选择今日日期，用户姓名中填写李德平，点击按检索条件删除。
 * 测试预期->1. 页面出现提示信息：是否删除日志信息！
 * 测试步骤->2. 选择提示信息中的是按钮
 * 测试预期->2. 页面出现提示信息：已删除日志信息！
 * 测试步骤->3. 点击提示信息中的确定按钮。
 * 测试预期->3. 跳转回最初页面，满足检索条件的日志信息均被删除。
 * 
 */

public class 验证日志信息按照检索条件删除功能是否可以实现_正常处理_1051 extends Selenium_Test_Base {
	
	public 验证日志信息按照检索条件删除功能是否可以实现_正常处理_1051(){
		super();
	}
	
	public 验证日志信息按照检索条件删除功能是否可以实现_正常处理_1051(WebDriver driver){
		super(driver);
	}
