 ...
 * 所属模块: 车型信息(#336)
 * 用例名称: 验证车型信息查询功能是否可实现-查看车型-正常操作
 * 前置条件: 能正常进入车型管理模块
 * 测试步骤->1. 点击查看车型按钮
 * 测试预期->1. 出现错误提示信息：请选择信息!
 * 测试步骤->2. 单击左键选择一条信息，点击查看车型
 * 测试预期->2. 出现查看车型窗口，含有各项车型信息。
 * 测试步骤->3. 点击右上角关闭按钮
 * 测试预期->3. 查看车型页面被正常关闭
 * 
 */

public class 验证车型信息查询功能是否可实现_查看车型_正常操作_3631 extends Selenium_Test_Base {
	
	public 验证车型信息查询功能是否可实现_查看车型_正常操作_3631(){
		super();
	}
	
	public 验证车型信息查询功能是否可实现_查看车型_正常操作_3631(WebDriver driver){
		super(driver);
	}
	...
	public void detailedOperation(){
		getElement(ElementEnum_车型信息_336.查看车型按钮).click();
		WebDriverConfiguration.operationInterval();
		...
		getElement(ElementEnum_车型信息_336.查看车型按钮).click();
		WebDriverConfiguration.operationInterval();
		assertTrue(getElement(ElementEnum_车型信息_336.弹出窗口保存按钮).getCssValue("display").equals("none"));
		
		getElement(ElementEnum_车型信息_336.弹出窗口右上角关闭按钮).click();
