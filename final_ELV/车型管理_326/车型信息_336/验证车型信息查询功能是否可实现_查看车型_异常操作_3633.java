package cn.edu.tju.scs.seql.final_ELV.车型管理_326.车型信息_336;

import static org.testng.Assert.assertEquals;
import static org.testng.Assert.*;

import org.openqa.selenium.*;
import org.testng.Assert;
import org.testng.annotations.*;

import cn.edu.tju.scs.seql.ELV.common.*;
import cn.edu.tju.scs.seql.element.*;

/**
 * 测试用例id: 3633
 * 所属产品: ELV环境合规系统（标准版）(#17)
 * 所属模块: 车型信息(#336)
 * 用例名称: 验证车型信息查询功能是否可实现-查看车型-异常操作
 * 前置条件: 能正常进入车型管理模块
 * 测试步骤->1. 直接点击查看车型
 * 测试预期->1. 弹出错误提示信息：请选择信息
 * 测试步骤->2. 单击左键选择一条信息，点击查看车型
 * 测试预期->2. 出现查看车型窗口，含有各项车型信息。
 * 测试步骤->3. 查看车型界面上的车型参数
 * 测试预期->3. 车型界面上的车型参数都不允许修改
 * 测试步骤->4. 点击右下角关闭按钮
 * 测试预期->4. 页面被正常关闭
 * 
 */

public class 验证车型信息查询功能是否可实现_查看车型_异常操作_3633 extends Selenium_Test_Base {
	
	public 验证车型信息查询功能是否可实现_查看车型_异常操作_3633(){
		super();
	}
	
	public 验证车型信息查询功能是否可实现_查看车型_异常操作_3633(WebDriver driver){
		super(driver);
	}
	
	@Test
	public void testNg() throws Exception {
		LoginAction.loginAction(driver,baseUrl);
		WebElement leftFrame=getElement(ElementEnum_车型信息_336.leftFrame);
		WebElement mainFrame=getElement(ElementEnum_车型信息_336.rightFrame);
	    driver.switchTo().frame(leftFrame);
	    operationInLeftFrame();
	    driver.switchTo().defaultContent();
		
		driver.switchTo().frame(mainFrame);
	    detailedOperation();
	}
	
	/**
	 * LeftFrame中的操作，主要是选择不同的功能
	 */
	public void operationInLeftFrame(){
		WebDriverConfiguration.operationInterval();
		getElement(ElementEnum_车型信息_336.一级目录名称).click();
		WebDriverConfiguration.operationInterval();
		getElement(ElementEnum_车型信息_336.二级目录名称).click();
		WebDriverConfiguration.operationInterval();
	}
	
	/**
	 * 具体的测试操作，在右边的mianFrame中进行
	 */
	public void detailedOperation(){
		getElement(ElementEnum_车型信息_336.查看车型按钮).click();
		WebDriverConfiguration.operationInterval();
		assertTrue(getElement(ElementEnum_车型信息_336.第一页提示信息).getText().contains("请选择信息"));
		getElement(ElementEnum_车型信息_336.第一页提示框确认按钮).click();
		WebDriverConfiguration.operationInterval();
		
		getElement(ElementEnum_车型信息_336.检索结果第一行车型中文名称).click();
		WebDriverConfiguration.operationInterval();
		getElement(ElementEnum_车型信息_336.查看车型按钮).click();
		WebDriverConfiguration.operationInterval();
		assertTrue(getElement(ElementEnum_车型信息_336.弹出窗口保存按钮).getCssValue("display").equals("none"));
		
		getElement(ElementEnum_车型信息_336.弹出窗口关闭按钮).click();
		
		
		
	}

	
	public String getDescription(){
		return "测试用例id: 3633\n"
				+" 所属产品: ELV环境合规系统（标准版）(#17)\n"
				+" 所属模块: 车型信息(#336)\n"
				+" 用例名称: 验证车型信息查询功能是否可实现-查看车型-异常操作\n"
				+" 前置条件: 能正常进入车型管理模块\n"
				+" 测试步骤->1. 直接点击查看车型\n"
				+" 测试预期->1. 弹出错误提示信息：请选择信息\n"
				+" 测试步骤->2. 单击左键选择一条信息，点击查看车型\n"
				+" 测试预期->2. 出现查看车型窗口，含有各项车型信息。\n"
				+" 测试步骤->3. 查看车型界面上的车型参数\n"
				+" 测试预期->3. 车型界面上的车型参数都不允许修改\n"
				+" 测试步骤->4. 点击右下角关闭按钮\n"
				+" 测试预期->4. 页面被正常关闭\n"
;
	}

}
