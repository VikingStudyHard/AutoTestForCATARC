 ...
 * 所属模块: 零部件BOM信息(#339)
 * 用例名称: 验证零部件BOM信息功能是否可实现-新增BOM-BOM树形修改操作
 * 前置条件: 能正常进入车型管理模块
 * 测试步骤->1. 点击新增BOM按钮
 ...
 * 测试预期->3. 弹出新的页面，出现各项需要填写的信息
 * 测试步骤->4. 供货编号为空的情况下
 * 测试预期->4. 弹出错误提示信息，供货编号不能为空
...
public class 验证零部件BOM信息功能是否可实现_新增BOM_BOM树形修改操作_3694 extends Selenium_Test_Base {
	
	public 验证零部件BOM信息功能是否可实现_新增BOM_BOM树形修改操作_3694(){
		super();
	}
	
	public 验证零部件BOM信息功能是否可实现_新增BOM_BOM树形修改操作_3694(WebDriver driver){
		super(driver);
	}
