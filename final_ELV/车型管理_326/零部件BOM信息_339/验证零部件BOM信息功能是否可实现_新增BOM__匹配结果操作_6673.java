 ...
 * 所属模块: 零部件BOM信息(#339)
 * 用例名称: 验证零部件BOM信息功能是否可实现-新增BOM--匹配结果操作
 * 前置条件: 能正常进入车型管理模块
 * 测试步骤->1. 点击新增BOM按钮
 * 测试预期->1. 出现查看BOM窗口，含有各项零部件BOM信息。
 * 测试步骤->2. 点击右上角的整体匹配按钮
...
public class 验证零部件BOM信息功能是否可实现_新增BOM__匹配结果操作_6673 extends Selenium_Test_Base {
	
	public 验证零部件BOM信息功能是否可实现_新增BOM__匹配结果操作_6673(){
		super();
	}
	
	public 验证零部件BOM信息功能是否可实现_新增BOM__匹配结果操作_6673(WebDriver driver){
		super(driver);
	}
	...
	public void detailedOperation(){
		...
		getElement(ElementEnum_零部件BOM信息_339.匹配结果子页面发送键).click();
		WebDriverConfiguration.operationInterval();
		String headName = getElement(ElementEnum_零部件BOM信息_339.页面标题).getText();
		if (state.equals("此mds不存在")){
			assertEquals(headName, "创建请求");
		}
		else if (state.equals("MDS未完成初审")){
			assertEquals(headName, "新建/编辑信息");
		}
		//else{
			//Assert.fail("未知的状态");
		//}
