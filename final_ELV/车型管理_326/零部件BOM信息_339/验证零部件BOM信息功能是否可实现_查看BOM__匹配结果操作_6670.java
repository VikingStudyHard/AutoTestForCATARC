 ...
 * 所属模块: 零部件BOM信息(#339)
 * 用例名称: 验证零部件BOM信息功能是否可实现-查看BOM--匹配结果操作
 * 前置条件: 能正常进入车型管理模块
 * 测试步骤->1. 单击左键选择一条零部件信息，点击查看BOM按钮
...
public class 验证零部件BOM信息功能是否可实现_查看BOM__匹配结果操作_6670 extends Selenium_Test_Base {
	
	public 验证零部件BOM信息功能是否可实现_查看BOM__匹配结果操作_6670(){
		super();
	}
	
	public 验证零部件BOM信息功能是否可实现_查看BOM__匹配结果操作_6670(WebDriver driver){
		super(driver);
	}
	...
	public void detailedOperation(){
		...
		for (int i = 1; i < eles.size(); i++){
			...
			if (canExpand){
				...
				else if (state.equals("此mds未完成初审")){
