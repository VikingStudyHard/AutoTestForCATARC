 ...
 * 所属模块: 扩展零部件(#341)
 * 用例名称: 验证扩展零部件信息确定功能是否可实现-内部查询-零部件名称
 * 前置条件: 能正常进入车型管理模块
 * 测试步骤->1. 点击检索按钮
 * 测试预期->1. 检索出全部信息
 * 测试步骤->2. 在零部件名称中填写M，点击检索按钮
 * 测试预期->2. 检索出零部件名称中含有M字段的信息
 * 测试步骤->3. 在零部件名称中填写MBR ASSY-FR SIDE RH，点击检索按钮
 * 测试预期->3. 检索出零部件名称中含有MBR ASSY-FR SIDE RH字段的信息
 * 
 */

public class 验证扩展零部件信息确定功能是否可实现_内部查询_零部件名称_4015 extends Selenium_Test_Base {
	
	public 验证扩展零部件信息确定功能是否可实现_内部查询_零部件名称_4015(){
		super();
	}
	
	public 验证扩展零部件信息确定功能是否可实现_内部查询_零部件名称_4015(WebDriver driver){
		super(driver);
	}
	...
	void query(String word){
		getElement(ElementEnum_扩展零部件_341.零部件名称输入框).clear();
		getElement(ElementEnum_扩展零部件_341.零部件名称输入框).sendKeys(word);
		...
		for (int i = 1; i < eles.size(); i++){
			String name = driver.findElement(By.xpath(ElementEnum_扩展零部件_341.检索结果第一行零部件名称.getName()
