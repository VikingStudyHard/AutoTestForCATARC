 ...
 * 所属模块: 扩展零部件(#341)
 * 用例名称: 验证扩展零部件信息导出报表功能是否可实现-报表内容是否正确
 * 前置条件: 能正常进入车型管理模块
 * 测试步骤->1. 任选一条有扩展零部件信息的车型信息，点击确定按钮
 * 测试预期->1. 跳转至新页面，页面包含所选信息的零部件变更信息，可输入查询参数来查询变更信息。
 * 测试步骤->2. 点击左上角的导出按钮
 * 测试预期->2. 所有变更信息被下载到本地，文件为.xlsx文件
 * 测试步骤->3. 点击查看报表内容
 * 测试预期->3. 整车生产企业指定因扩展而增加的零部件名称，基于零部件名称，由系统自动导出该零部件的零部件编号、供应商名称及材料数据报表编号/版本信息
 * 
 */

public class 验证扩展零部件信息导出报表功能是否可实现_报表内容是否正确_13643 extends Selenium_Test_Base {
	
	public 验证扩展零部件信息导出报表功能是否可实现_报表内容是否正确_13643(){
		super();
	}
	
	public 验证扩展零部件信息导出报表功能是否可实现_报表内容是否正确_13643(WebDriver driver){
		super(driver);
	}
