...
public class 验证扩展零部件信息查询功能是否可实现_删除操作_4006 extends Selenium_Test_Base {
	
	public 验证扩展零部件信息查询功能是否可实现_删除操作_4006(){
		super();
	}
	
	public 验证扩展零部件信息查询功能是否可实现_删除操作_4006(WebDriver driver){
		super(driver);
	}
	...
	public void detailedOperation(){
		...
		for (int i = 1; i < eles.size(); i++){
			driver.findElement(By.xpath(ElementEnum_扩展零部件_341.表格第一行.getName()
	...
	private boolean doInSubpage(){
		...
		if (eles.size() < 2){
			...
			getElement(ElementEnum_扩展零部件_341.删除按钮).click();
			String message = getElement(ElementEnum_扩展零部件_341.子页面未选择信息错误提示).getText();
			assertEquals(message, "请选择信息!");
			getElement(ElementEnum_扩展零部件_341.子页面未选择信息错误提示确定键).click();
			WebDriverConfiguration.operationInterval();
			
			String name = getElement(ElementEnum_扩展零部件_341.检索结果第一行零部件名称).getText();
			
			getElement(ElementEnum_扩展零部件_341.检索结果第一行).click();
			getElement(ElementEnum_扩展零部件_341.删除按钮).click();
			message = getElement(ElementEnum_扩展零部件_341.子页面未选择信息错误提示).getText();
			assertEquals(message, "确实要删除这些记录吗?");
			getElement(ElementEnum_扩展零部件_341.删除取消键).click();
			WebDriverConfiguration.operationInterval();
			assertTrue(isRecordExists(name));
			
			getElement(ElementEnum_扩展零部件_341.删除按钮).click();
			message = getElement(ElementEnum_扩展零部件_341.子页面未选择信息错误提示).getText();
			assertEquals(message, "确实要删除这些记录吗?");
			getElement(ElementEnum_扩展零部件_341.删除确定键).click();
			WebDriverConfiguration.operationInterval();
			assertFalse(isRecordExists(name));
			return true;
		}
	}
	...
	private boolean isRecordExists(String time){
		...
		if (eles.size() == 0)
			return false;
		for (int i = 1; i < eles.size(); i++){
			String t = driver.findElement(By.xpath(ElementEnum_扩展零部件_341.检索结果第一行零部件名称.getName()
