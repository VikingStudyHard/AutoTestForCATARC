 ...
 * 所属模块: 扩展零部件(#341)
 * 用例名称: 验证扩展零部件信息查询功能是否可实现-车型名称检索
 * 前置条件: 能正常进入车型管理模块
 * 测试步骤->1. 在扩展零部件信息的查询条件：车型名称中填写1，点击检索按钮
 * 测试预期->1. 符合条件的信息被检索出
 * 测试步骤->2. 在扩展零部件信息的查询条件：车型名称中填写ELVT，点击检索按钮
 * 测试预期->2. 符合条件的信息被检索出
 * 测试步骤->3. 在扩展零部件信息的查询条件：车型名称中填写ELVTEST，点击检索按钮
 * 测试预期->3. 车型名称（中文或英文）名称中为ELVTEST的所有扩展零部件信息被检索出来
 * 测试步骤->4. 在扩展零部件信息的查询条件：车型名称中填写匹配测试，点击检索按钮
 * 测试预期->4. 符合条件的信息被检索出
 * 
 */

public class 验证扩展零部件信息查询功能是否可实现_车型名称检索_4019 extends Selenium_Test_Base {
	
	public 验证扩展零部件信息查询功能是否可实现_车型名称检索_4019(){
		super();
	}
	
	public 验证扩展零部件信息查询功能是否可实现_车型名称检索_4019(WebDriver driver){
		super(driver);
	}
	...
	private void query(String word){
		getElement(ElementEnum_扩展零部件_341.车型名称查询).clear();
		getElement(ElementEnum_扩展零部件_341.车型名称查询).sendKeys(word);
		...
		for (int i = 1; i < eles.size(); i++){
			String cname = driver.findElement(By.xpath(ElementEnum_扩展零部件_341.表格第一行车型中文名称.getName()
					.replaceAll("tr\\[2\\]", "tr[" + (i + 1) + "]"))).getText();
			String ename = driver.findElement(By.xpath(ElementEnum_扩展零部件_341.表格第一行车型英文名称.getName()
					.replaceAll("tr\\[2\\]", "tr[" + (i + 1) + "]"))).getText();
