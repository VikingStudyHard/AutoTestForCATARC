...
public class 验证变更零部件下方功能栏按钮是否可以实现_含异常操作_4004 extends Selenium_Test_Base {
	
	public 验证变更零部件下方功能栏按钮是否可以实现_含异常操作_4004(){
		super();
	}
	
	public 验证变更零部件下方功能栏按钮是否可以实现_含异常操作_4004(WebDriver driver){
		super(driver);
	}
	...
	public void detailedOperation(){
		waitingForLoading();
		getElement(ElementEnum_扩展零部件_341.下一页).click();
		waitingForLoading();
		Integer[] n = getRecordsNumber();
		Integer count = n[1] - n[0] + 1;
		checkTable(2, getElements(ElementEnum_扩展零部件_341.表格所有项), getRecordsNumber(), count);
		
		//这个上一页点击是因为下一页按钮的提示标签遮住了最后一页的按钮导致不能直接点击
		getElement(ElementEnum_扩展零部件_341.上一页).click();
		waitingForLoading();
		getElement(ElementEnum_扩展零部件_341.最后一页).click();
		waitingForLoading();
		
		//取出总页数，从而得出最后一页的页码
		String pageContent = getElement(ElementEnum_扩展零部件_341.总页数).getText();
		Integer page = Integer.valueOf(pageContent.substring(pageContent.indexOf("共") + 2, pageContent.length() - 2));
		System.out.println("[" + page + "]");
		checkTable(page, getElements(ElementEnum_扩展零部件_341.表格所有项), getRecordsNumber(), count);
		getElement(ElementEnum_扩展零部件_341.页码输入框).clear();
		getElement(ElementEnum_扩展零部件_341.页码输入框).sendKeys("1");
		getElement(ElementEnum_扩展零部件_341.页码输入框).sendKeys(Keys.ENTER);
		WebDriverConfiguration.operationInterval();
		checkTable(1, getElements(ElementEnum_扩展零部件_341.表格所有项), getRecordsNumber(), count);
		getElement(ElementEnum_扩展零部件_341.页码输入框).clear();
		getElement(ElementEnum_扩展零部件_341.页码输入框).sendKeys("2000000");
		getElement(ElementEnum_扩展零部件_341.页码输入框).sendKeys(Keys.ENTER);
		WebDriverConfiguration.operationInterval();
		checkTable(page, getElements(ElementEnum_扩展零部件_341.表格所有项), getRecordsNumber(), count);
	}
	...
	private void checkTable(Integer page, List<WebElement> l, Integer[] se, Integer count){
		...
		for (int i = 0; i < l.size() - 1; i++){
			String xpath = ElementEnum_扩展零部件_341.表格第一行序号.getName().replaceAll("tr\\[2\\]", "tr[" + (i + 2) + "]");
	...
	private Integer[] getRecordsNumber(){
		Integer start, end;
		String text = getElement(ElementEnum_扩展零部件_341.右下角数量提示).getText();
