 ...
 * 所属模块: 变更零部件(#340)
 * 用例名称: 验证变更零部件信息查询功能是否可实现-创建时间检索
 * 前置条件: 能正常进入车型管理模块
...
public class 验证变更零部件信息查询功能是否可实现_创建时间检索_3989 extends Selenium_Test_Base {
	
	public 验证变更零部件信息查询功能是否可实现_创建时间检索_3989(){
		super();
	}
	
	public 验证变更零部件信息查询功能是否可实现_创建时间检索_3989(WebDriver driver){
		super(driver);
	}
	...
	private void query(String start, String end){
		((JavascriptExecutor)driver).executeScript("arguments[0].removeAttribute('readonly')", getElement(ElementEnum_变更零部件_340.创建时间开始));
		((JavascriptExecutor)driver).executeScript("arguments[0].removeAttribute('readonly')", getElement(ElementEnum_变更零部件_340.创建时间截止));
		getElement(ElementEnum_变更零部件_340.创建时间开始).clear();
		getElement(ElementEnum_变更零部件_340.创建时间截止).clear();
		Calendar startTime = null;
		Calendar endTime = null;
		if (start != null){
			getElement(ElementEnum_变更零部件_340.创建时间开始).sendKeys(start);
		...
		if (end != null){
			getElement(ElementEnum_变更零部件_340.创建时间截止).sendKeys(end);
			String[] t = end.split("-");
			//System.out.println(t[0] + " " + t[1] + " " + t[2]);
		...
		if (startTime != null && endTime != null && startTime.compareTo(endTime) > 0){
			String text = getElement(ElementEnum_变更零部件_340.时间错误提示).getText();
			assertEquals(text, "开始时间早于结束时间!");
			WebDriverConfiguration.operationInterval();
			getElement(ElementEnum_变更零部件_340.时间错误提示确定键).click();
			return;
		}
		...
		for (int i = 1; i < eles.size(); i++){
			String time = driver.findElement(By.xpath(ElementEnum_变更零部件_340.表格第一行时间.getName()
