 ...
 * 所属模块: 变更零部件(#340)
 * 用例名称: 验证变更零部件信息查询功能是否可实现-新增变更-异常操作
 * 前置条件: 能正常进入车型管理模块
 * 测试步骤->1. 点击新增变更按钮
 * 测试预期->1. 跳转新页面，含有可填写或查询的变更部件信息
 * 测试步骤->2. 点击新页面中变更前ID的查找按钮
 * 测试预期->2. 跳转至查找MDS信息页面，即可填写查询参数查找MDS信息
 * 测试步骤->3. 勾选任意一条信息，点击确定按钮
 * 测试预期->3. 回到新增变更页面，变更前ID为刚勾选信息的ID
 * 测试步骤->4. 在备注信息中填写test，点击保存
 * 测试预期->4. 出现提示信息：请填写变更后报告编号
 * 测试步骤->5. 点击新页面中的变更后ID查找按钮
 * 测试预期->5. 跳转至查找MDS信息页面，即可填写查询参数查找MDS信息
 * 测试步骤->6. 勾选同一条信息，点击确定按钮
 * 测试预期->6. 回到新增变更页面，变更后ID为刚勾选信息的ID
 * 测试步骤->7. 在备注信息中填写test，点击保存
 * 测试预期->7. 出现错误信息：相同的变更后零部件已经被使用过，请重新选择
 * 
 */

public class 验证变更零部件信息查询功能是否可实现_新增变更_异常操作_3997 extends Selenium_Test_Base {
	
	public 验证变更零部件信息查询功能是否可实现_新增变更_异常操作_3997(){
		super();
	}
	
	public 验证变更零部件信息查询功能是否可实现_新增变更_异常操作_3997(WebDriver driver){
		super(driver);
	}
