 ...
 * 所属模块: 变更零部件(#340)
 * 用例名称: 验证变更零部件信息查询功能是否可实现-修改变更-正常操作
 * 前置条件: 能正常进入车型管理模块
 * 测试步骤->1. 勾选一条变更信息，点击修改变更按钮
 * 测试预期->1. 跳转新页面，含有可填写或查询的变更部件信息
 * 测试步骤->2. 点击新页面中变更前ID的查找按钮
 * 测试预期->2. 跳转至查找MDS信息页面，即可填写查询参数查找MDS信息
 * 测试步骤->3. 勾选不同于变更后的ID且未变更过的信息，点击确定按钮
 * 测试预期->3. 回到新增变更页面，变更前ID为刚勾选信息的ID
 * 测试步骤->4. 点击保存按钮
 * 测试预期->4. 出现提示信息：数据已保存。页面回到零部件变更信息界面，含有刚刚提交的更改信息
 * 测试步骤->5. 点击查看修改过的信息
 * 测试预期->5. 信息被成功修改
 * 
 */

public class 验证变更零部件信息查询功能是否可实现_修改变更_正常操作_4000 extends Selenium_Test_Base {
	
	public 验证变更零部件信息查询功能是否可实现_修改变更_正常操作_4000(){
		super();
	}
	
	public 验证变更零部件信息查询功能是否可实现_修改变更_正常操作_4000(WebDriver driver){
		super(driver);
	}
