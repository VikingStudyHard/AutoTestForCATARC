 ...
 * 所属模块: 变更零部件(#340)
 * 用例名称: 验证变更零部件信息确定功能是否可实现-内部查询-变更前供货编号
 * 前置条件: 能正常进入车型管理模块
 * 测试步骤->1. 在变更前供货编号中填写7511，点击检索按钮
 * 测试预期->1. 未检索出符合条件的信息
 * 测试步骤->2. 在变更前供货编号中填写75111LA220，点击检索按钮
 * 测试预期->2. 检索出变更前供货编号为75111LA220的信息
 * 
 */

public class 验证变更零部件信息确定功能是否可实现_内部查询_变更前供货编号_3994 extends Selenium_Test_Base {
	
	public 验证变更零部件信息确定功能是否可实现_内部查询_变更前供货编号_3994(){
		super();
	}
	
	public 验证变更零部件信息确定功能是否可实现_内部查询_变更前供货编号_3994(WebDriver driver){
		super(driver);
	}
	...
	void query(String word){
		getElement(ElementEnum_变更零部件_340.变更前供货编号输入框).clear();
		getElement(ElementEnum_变更零部件_340.变更前供货编号输入框).sendKeys(word);
		...
		for (int i = 1; i < eles.size(); i++){
			String name = driver.findElement(By.xpath(ElementEnum_变更零部件_340.检索结果第一行变更前供货编号.getName()
