...
public class 验证变更零部件信息查询功能是否可实现_导出操作_4002 extends Selenium_Test_Base {
	
	public 验证变更零部件信息查询功能是否可实现_导出操作_4002(){
		super();
	}
	
	public 验证变更零部件信息查询功能是否可实现_导出操作_4002(WebDriver driver){
		super(driver);
	}
	...
	public void detailedOperation(){
		...
		WebDriverConfiguration.operationInterval();
		int count = getElements(ElementEnum_变更零部件_340.检索结果所有行).size();
		boolean result = downloadOperation(getElement(ElementEnum_变更零部件_340.导出按钮));
		if (count >= 2){
			assertTrue(result);
		}
		else{
