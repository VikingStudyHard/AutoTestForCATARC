 ...
 * 所属模块: BOM分发结果(#338)
 * 用例名称: 验证bom分发结果下方功能栏按钮是否可以实现-含异常操作
 * 前置条件: 能正常进入车型信息界面
 * 测试步骤->1. 点击bom分发结果界面最下方的翻页功能栏中的下一页按钮
 * 测试预期->1. 页面信息跳转至下一页，页面其余内容正常显示。
 * 测试步骤->2. 点击bom分发结果界面最下方的翻页功能栏中的最后一页按钮
 * 测试预期->2. 页面信息跳转至最后一页，页面其余内容正常显示。
 * 测试步骤->3. 点击bom分发结果界面最下方的翻页功能栏中的填写框填写1，按回车键。
 * 测试预期->3. 页面信息跳转至第1页，页面其余内容正常显示。
 * 测试步骤->4. 点击bom分发结果界面最下方的翻页功能栏中的填写框填写2000000，按回车键。
 * 测试预期->4. 页面信息跳转至最后一页，页面其余内容正常显示。
 * 
 */

public class 验证bom分发结果下方功能栏按钮是否可以实现_含异常操作_3673 extends Selenium_Test_Base {
	
	public 验证bom分发结果下方功能栏按钮是否可以实现_含异常操作_3673(){
		super();
	}
	
	public 验证bom分发结果下方功能栏按钮是否可以实现_含异常操作_3673(WebDriver driver){
		super(driver);
	}
	...
	public void detailedOperation(){
		/*
		 * 翻页功能在变更零部件管理模块已经缩过测试，可以不进行重复的测试
		 */
		...
		//取出总页数，从而得出最后一页的页码
		String pageContent = getElement(ElementEnum_BOM分发结果_338.总页数).getText();
		...
		getElement(ElementEnum_BOM分发结果_338.下一页).click();
		waitingForLoading();
		Integer[] n = getRecordsNumber();
		Integer count = n[1] - n[0] + 1;
		checkTable(2, getElements(ElementEnum_BOM分发结果_338.表格所有行), getRecordsNumber(), count);
		
		//这个上一页点击是因为下一页按钮的提示标签遮住了最后一页的按钮导致不能直接点击
		getElement(ElementEnum_BOM分发结果_338.上一页).click();
		waitingForLoading();
		getElement(ElementEnum_BOM分发结果_338.最后一页).click();
		waitingForLoading();
		
		
		checkTable(page, getElements(ElementEnum_BOM分发结果_338.表格所有行), getRecordsNumber(), count);
		getElement(ElementEnum_BOM分发结果_338.页码输入框).clear();
		getElement(ElementEnum_BOM分发结果_338.页码输入框).sendKeys("1");
		getElement(ElementEnum_BOM分发结果_338.页码输入框).sendKeys(Keys.ENTER);
		WebDriverConfiguration.operationInterval();
		checkTable(1, getElements(ElementEnum_BOM分发结果_338.表格所有行), getRecordsNumber(), count);
		getElement(ElementEnum_BOM分发结果_338.页码输入框).clear();
		getElement(ElementEnum_BOM分发结果_338.页码输入框).sendKeys("2000000");
		getElement(ElementEnum_BOM分发结果_338.页码输入框).sendKeys(Keys.ENTER);
		WebDriverConfiguration.operationInterval();
		checkTable(page, getElements(ElementEnum_BOM分发结果_338.表格所有行), getRecordsNumber(), count);
	}
	...
	private void checkTable(Integer page, List<WebElement> l, Integer[] se, Integer count){
		...
		for (int i = 0; i < l.size() - 1; i++){
			String xpath = ElementEnum_BOM分发结果_338.表格第一行序号.getName().replaceAll("tr\\[2\\]", "tr[" + (i + 2) + "]");
	...
	private Integer[] getRecordsNumber(){
		Integer start, end;
		String text = getElement(ElementEnum_BOM分发结果_338.右下角数量提示).getText();
	...
	private void waitingForLoading(){
		...
		} while((!getElement(ElementEnum_BOM分发结果_338.Loading框).getCssValue("display").equals("none")));
	}
