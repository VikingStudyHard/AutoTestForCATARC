 ...
 * 所属模块: 禁限用物质豁免分析(#806)
 * 用例名称: 验证禁限用物质豁免分析 导出有害物质清单内容是否正确(1)
 ...
 * 测试步骤->3. 点击查看清单内容
 * 测试预期->3. 为了避免重复（如导出铜合金中含有发动机、变速器和空调压缩机的轴瓦和轴套），整个表格的导出，应先导出指定的零部件或材料，然后根据规则从系统中导出除去指定零部件或材料外，并满足条件的信息
 * 
 */

public class 验证禁限用物质豁免分析_导出有害物质清单内容是否正确_1__13594 extends Selenium_Test_Base {
	
	public 验证禁限用物质豁免分析_导出有害物质清单内容是否正确_1__13594(){
		super();
	}
	
	public 验证禁限用物质豁免分析_导出有害物质清单内容是否正确_1__13594(WebDriver driver){
		super(driver);
	}
