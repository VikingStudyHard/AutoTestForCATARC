 ...
 * 用例名称: 验证禁用物质豁免分析模块
 * 前置条件: 能整车进入综合分析模块
 * 测试步骤->1. 进入综合分析模块，点击禁限用物质豁免分析
 * 测试预期->1. 页面右侧出现基本信息界面，其中质量一项单位为g
 * 
 */

public class 验证禁用物质豁免分析模块_7544 extends Selenium_Test_Base {
	
	public 验证禁用物质豁免分析模块_7544(){
		super();
	}
	
	public 验证禁用物质豁免分析模块_7544(WebDriver driver){
		super(driver);
	}
