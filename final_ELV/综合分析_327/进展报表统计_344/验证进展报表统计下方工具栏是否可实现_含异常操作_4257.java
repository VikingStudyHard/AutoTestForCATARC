 ...
 * 所属模块: 进展报表统计(#344)
 * 用例名称: 验证进展报表统计下方工具栏是否可实现-含异常操作
 * 前置条件: 能正常进入综合分析模块
 * 测试步骤->1. 点击进展报表统计界面最下方的翻页功能栏中的下一页按钮
 * 测试预期->1. 页面信息跳转至下一页，页面其余内容正常显示。
 * 测试步骤->2. 点击进展报表统计界面最下方的翻页功能栏中的最后一页按钮
 * 测试预期->2. 页面信息跳转至最后一页，页面其余内容正常显示。
 * 测试步骤->3. 点击进展报表统计界面最下方的翻页功能栏中的填写框填写1，按回车键。
 * 测试预期->3. 页面信息跳转至第1页，页面其余内容正常显示。
 * 测试步骤->4. 点击进展报表统计界面最下方的翻页功能栏中的填写框填写2000000，按回车键。
 * 测试预期->4. 页面信息跳转至最后一页，页面其余内容正常显示。
 * 
 */

public class 验证进展报表统计下方工具栏是否可实现_含异常操作_4257 extends Selenium_Test_Base {
	
	public 验证进展报表统计下方工具栏是否可实现_含异常操作_4257(){
		super();
	}
	
	public 验证进展报表统计下方工具栏是否可实现_含异常操作_4257(WebDriver driver){
		super(driver);
	}
	...
	public void detailedOperation(){
		waitingForLoading();
		getElement(ElementEnum_进展报表统计_344.下一页).click();
		waitingForLoading();
		Integer[] n = getRecordsNumber();
		Integer count = n[1] - n[0] + 1;
		checkTable(2, getElements(ElementEnum_进展报表统计_344.表格所有项), getRecordsNumber(), count);
		
		//这个上一页点击是因为下一页按钮的提示标签遮住了最后一页的按钮导致不能直接点击
		getElement(ElementEnum_进展报表统计_344.上一页).click();
		waitingForLoading();
		getElement(ElementEnum_进展报表统计_344.最后一页).click();
		waitingForLoading();
		
		//取出总页数，从而得出最后一页的页码
		String pageContent = getElement(ElementEnum_进展报表统计_344.总页数).getText();
		Integer page = Integer.valueOf(pageContent.substring(pageContent.indexOf("共") + 2, pageContent.length() - 2));
		System.out.println("[" + page + "]");
		checkTable(page, getElements(ElementEnum_进展报表统计_344.表格所有项), getRecordsNumber(), count);
		getElement(ElementEnum_进展报表统计_344.页码输入框).clear();
		getElement(ElementEnum_进展报表统计_344.页码输入框).sendKeys("1");
		getElement(ElementEnum_进展报表统计_344.页码输入框).sendKeys(Keys.ENTER);
		WebDriverConfiguration.operationInterval();
		checkTable(1, getElements(ElementEnum_进展报表统计_344.表格所有项), getRecordsNumber(), count);
		getElement(ElementEnum_进展报表统计_344.页码输入框).clear();
		getElement(ElementEnum_进展报表统计_344.页码输入框).sendKeys("2000000");
		getElement(ElementEnum_进展报表统计_344.页码输入框).sendKeys(Keys.ENTER);
		WebDriverConfiguration.operationInterval();
		checkTable(page, getElements(ElementEnum_进展报表统计_344.表格所有项), getRecordsNumber(), count);
	}
	...
	private void checkTable(Integer page, List<WebElement> l, Integer[] se, Integer count){
		...
		for (int i = 0; i < l.size() - 1; i++){
			String xpath = ElementEnum_进展报表统计_344.表格第一行序号.getName().replaceAll("tr\\[2\\]", "tr[" + (i + 2) + "]");
	...
	private Integer[] getRecordsNumber(){
		Integer start, end;
		String text = getElement(ElementEnum_进展报表统计_344.右下角数量提示).getText();
	...
	private void waitingForLoading(){
		...
		} while((!getElement(ElementEnum_进展报表统计_344.Loading框).getAttribute("class").equals("")));
	}
