 ...
 * 所属模块: 进展报表统计(#344)
 * 用例名称: 验证进展报表统计查看车型功能是否可实现-异常操作
 * 前置条件: 能正常进入综合分析模块
 * 测试步骤->1. 单击左键选择一条车型信息，点击查看车型按钮
 * 测试预期->1. 跳出新页面，页面名称为查看车型，含有该车型的各项信息内容，含有已填写的参数信息
 * 测试步骤->2. 修改个别参数信息，如将品牌中文名称改为reset，整车质量改为0，点击关闭按钮
 * 测试预期->2. 查看车型页面被正常关闭，页面回到进展报表统计首页
 * 测试步骤->3. 单击左键选择更改过的车型信息，点击查看车型按钮
 * 测试预期->3. 跳出新页面，页面名称为查看车型，含有该车型的各项信息内容，含有已填写的参数信息，信息未被更改
 * 测试步骤->4. 点击页面右上角的关闭键
 * 测试预期->4. 查看车型页面被正常关闭，页面回到进展报表统计首页
 * 
 */

public class 验证进展报表统计查看车型功能是否可实现_异常操作_4075 extends Selenium_Test_Base {
	
	public 验证进展报表统计查看车型功能是否可实现_异常操作_4075(){
		super();
	}
	
	public 验证进展报表统计查看车型功能是否可实现_异常操作_4075(WebDriver driver){
		super(driver);
	}
	...
	public void detailedOperation(){
		...
		getElement(ElementEnum_进展报表统计_344.品牌中文名称).clear();
		String newValue = bcname == "testArg" ? "testArg2" : "testArg";
		getElement(ElementEnum_进展报表统计_344.品牌中文名称).sendKeys(newValue);
		getElement(ElementEnum_进展报表统计_344.关闭按钮).click();
		getElement(ElementEnum_进展报表统计_344.表格第一行).click();
		getElement(ElementEnum_进展报表统计_344.查看车型按钮).click();
		assertEquals(bcname, getElement(ElementEnum_进展报表统计_344.品牌中文名称).getAttribute("value"));
		getElement(ElementEnum_进展报表统计_344.关闭叉叉).click();
	}
