 ...
 * 所属模块: 整车RRR计算(#346)
 * 用例名称: 测试车型回收利用率报告中的RRR报表内容是否正确（3）
 ...
 * 测试预期->5. 首先指定列入mD的零部件名称（应排除预处理阶段的零部件），并对列入mD的理由进行简要说明（主要包括该零部件的材料、连接方式、拆解方法及相应材料适用的可再利用技术名称/编号）
 * 
 */

public class 测试车型回收利用率报告中的RRR报表内容是否正确_3__13648 extends Selenium_Test_Base {
	
	public 测试车型回收利用率报告中的RRR报表内容是否正确_3__13648(){
		super();
	}
	
	public 测试车型回收利用率报告中的RRR报表内容是否正确_3__13648(WebDriver driver){
		super(driver);
	}
