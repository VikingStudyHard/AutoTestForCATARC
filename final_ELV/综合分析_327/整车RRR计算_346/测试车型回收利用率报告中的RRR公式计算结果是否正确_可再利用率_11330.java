 ...
 * 所属模块: 整车RRR计算(#346)
 * 用例名称: 测试车型回收利用率报告中的RRR公式计算结果是否正确-可再利用率
 ...
 * 测试步骤->6. 根据公式RCYC=[(mP+mD+mM+mTr)/mV]×100%计算得到可再利用率。
 * 测试预期->6. 计算结果为5.8%，计算结果正确无误。
 * 
 */

public class 测试车型回收利用率报告中的RRR公式计算结果是否正确_可再利用率_11330 extends Selenium_Test_Base {
	
	public 测试车型回收利用率报告中的RRR公式计算结果是否正确_可再利用率_11330(){
		super();
	}
	
	public 测试车型回收利用率报告中的RRR公式计算结果是否正确_可再利用率_11330(WebDriver driver){
		super(driver);
	}
