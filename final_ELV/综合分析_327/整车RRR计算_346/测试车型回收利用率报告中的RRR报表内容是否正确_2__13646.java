 ...
 * 所属模块: 整车RRR计算(#346)
 * 用例名称: 测试车型回收利用率报告中的RRR报表内容是否正确（2）
 ...
 * 测试预期->5. 如拆解阶段部件数量多余10个，首先应将主表填报完整，并在“附表1拆解零部件清单”中同时导出该10个零部件，保证各个阶段的完整性，其余阶段类似处理
 * 
 */

public class 测试车型回收利用率报告中的RRR报表内容是否正确_2__13646 extends Selenium_Test_Base {
	
	public 测试车型回收利用率报告中的RRR报表内容是否正确_2__13646(){
		super();
	}
	
	public 测试车型回收利用率报告中的RRR报表内容是否正确_2__13646(WebDriver driver){
		super(driver);
	}
