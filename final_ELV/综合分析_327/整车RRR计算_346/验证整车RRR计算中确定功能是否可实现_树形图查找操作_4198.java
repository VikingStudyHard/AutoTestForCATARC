 ...
 * 所属模块: 整车RRR计算(#346)
 * 用例名称:  验证整车RRR计算中确定功能是否可实现-树形图查找操作
 * 前置条件: 能正常进入综合分析模块
...
public class 验证整车RRR计算中确定功能是否可实现_树形图查找操作_4198 extends Selenium_Test_Base {
	
	public 验证整车RRR计算中确定功能是否可实现_树形图查找操作_4198(){
		super();
	}
	
	public 验证整车RRR计算中确定功能是否可实现_树形图查找操作_4198(WebDriver driver){
		super(driver);
	}
