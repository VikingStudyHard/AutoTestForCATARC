 ...
 * 所属模块: 整车RRR计算(#346)
 * 用例名称: 测试车型回收利用率报告中的RRR报表内容是否正确（5）
 ...
 * 测试预期->5. 除预处理阶段、拆解阶段、金属分离阶段的零部件之外的整车所有可再利用的非金属材料。在对CAMDS中5-8类材料进行mTr的标记时，标记mTr后，根据CPTL清单，以VDA材料分类为依据，弹出“材料范围”相应下拉选择框，以明确该材料的具体名称（对应CPTL清单中的“材料范围”列）
 * 
 */

public class 测试车型回收利用率报告中的RRR报表内容是否正确_5__13651 extends Selenium_Test_Base {
	
	public 测试车型回收利用率报告中的RRR报表内容是否正确_5__13651(){
		super();
	}
	
	public 测试车型回收利用率报告中的RRR报表内容是否正确_5__13651(WebDriver driver){
		super(driver);
	}
