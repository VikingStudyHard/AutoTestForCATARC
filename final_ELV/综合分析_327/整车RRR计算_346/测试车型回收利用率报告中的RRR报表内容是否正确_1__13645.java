 ...
 * 所属模块: 整车RRR计算(#346)
 * 用例名称: 测试车型回收利用率报告中的RRR报表内容是否正确（1）
 ...
 * 测试预期->5. 各阶段数值的保留位数见RRR合规报表模板，即除mV、mP、mD、mM、mTr、mTe、RCYC和RCOV的计算结果精确到小数点后1位外，其余数值统一保留4位小数
 * 
 */

public class 测试车型回收利用率报告中的RRR报表内容是否正确_1__13645 extends Selenium_Test_Base {
	
	public 测试车型回收利用率报告中的RRR报表内容是否正确_1__13645(){
		super();
	}
	
	public 测试车型回收利用率报告中的RRR报表内容是否正确_1__13645(WebDriver driver){
		super(driver);
	}
