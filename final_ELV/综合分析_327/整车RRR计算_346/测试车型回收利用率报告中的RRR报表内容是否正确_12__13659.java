 ...
 * 所属模块: 整车RRR计算(#346)
 * 用例名称: 测试车型回收利用率报告中的RRR报表内容是否正确（12）
 ...
 * 测试预期->5. 除零部件根节点外，其余所有子零部件的数量需体现，零部件根节点、半成品、材料节点，数量空白
 * 
 */

public class 测试车型回收利用率报告中的RRR报表内容是否正确_12__13659 extends Selenium_Test_Base {
	
	public 测试车型回收利用率报告中的RRR报表内容是否正确_12__13659(){
		super();
	}
	
	public 测试车型回收利用率报告中的RRR报表内容是否正确_12__13659(WebDriver driver){
		super(driver);
	}
