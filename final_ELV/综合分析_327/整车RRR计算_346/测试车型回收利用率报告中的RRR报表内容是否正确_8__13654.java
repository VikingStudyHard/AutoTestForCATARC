 ...
 * 所属模块: 整车RRR计算(#346)
 * 用例名称: 测试车型回收利用率报告中的RRR报表内容是否正确（8）
 ...
 * 测试预期->5. 除预处理阶段、拆解阶段、金属分离阶段、可再利用阶段的零部件材料之外的整车所有可能量回收利用的非金属材料，对CAMDS中5-8类材料进行mTe的标记时，标记mTe后，根据CPTL清单，以VDA材料分类为依据，弹出“材料范围”相应下拉选择框，类似mTr。以明确该材料的具体名称（对应CPTL清单中的“材料范围”列）
 * 
 */

public class 测试车型回收利用率报告中的RRR报表内容是否正确_8__13654 extends Selenium_Test_Base {
	
	public 测试车型回收利用率报告中的RRR报表内容是否正确_8__13654(){
		super();
	}
	
	public 测试车型回收利用率报告中的RRR报表内容是否正确_8__13654(WebDriver driver){
		super(driver);
	}
