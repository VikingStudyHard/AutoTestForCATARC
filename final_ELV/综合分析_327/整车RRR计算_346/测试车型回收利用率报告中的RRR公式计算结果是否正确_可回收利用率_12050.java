 ...
 * 所属模块: 整车RRR计算(#346)
 * 用例名称: 测试车型回收利用率报告中的RRR公式计算结果是否正确-可回收利用率
 ...
 * 测试步骤->5. 查看数据表中的重点信息内容是否填写正确
 * 测试预期->5. 表中mp为0.0，md为0.2，mM为29.2，mTr为0.8，mV为524.4,，各项信息内容填写无误。
 * 测试步骤->6. 根据公式RCOV=[(mP+mD+mM+mTr+mTe)/mV]×100%计算得到可再利用率。
 * 测试预期->6. 计算结果为5.8%，计算结果正确无误。
 * 
 */

public class 测试车型回收利用率报告中的RRR公式计算结果是否正确_可回收利用率_12050 extends Selenium_Test_Base {
	
	public 测试车型回收利用率报告中的RRR公式计算结果是否正确_可回收利用率_12050(){
		super();
	}
	
	public 测试车型回收利用率报告中的RRR公式计算结果是否正确_可回收利用率_12050(WebDriver driver){
		super(driver);
	}
