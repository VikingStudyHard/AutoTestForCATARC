 ...
 * 所属模块: 零部件RRR计算(#345)
 * 用例名称: 验证零部件RRR计算的查看标注记录功能是否可以实现-打开查询窗口-ID检索
 * 前置条件: 能正确进入综合分析模块
 * 测试步骤->1. 勾选一项MDS信息，点击零部件RRR计算页面中的查看标注记录按钮
 * 测试预期->1. 跳转至新页面，页面为查看的数据标注记录，可查看所选信息的数据标注记录
 * 测试步骤->2. 点击新页面右上角的查询窗口按钮
 * 测试预期->2. 出现查询窗口
 * 测试步骤->3. 在查询条件：ID中填写CA，点击检索按钮
 * 测试预期->3. 出现提示信息：请输入开始时间
 * 测试步骤->4. 选择开始时间为2016年3月18日，点击检索按钮
 * 测试预期->4. 出现提示信息：请输入结束时间
 * 测试步骤->5. 选择结束时间为2016年5月28日，点击检索按钮
 * 测试预期->5. 无满足条件的信息被检索出来
 * 测试步骤->6. 在查询条件：ID中填写CA_5_13182570 / 1.0，点击检索按钮
 * 测试预期->6. 无符合条件的信息被检索出来
 * 测试步骤->7. 在查询条件：ID中填写CA_5_13182570，点击检索按钮
 * 测试预期->7. ID为CA_5_13182570的信息被检索出来
 * 
 */

public class 验证零部件RRR计算的查看标注记录功能是否可以实现_打开查询窗口_ID检索_4249 extends Selenium_Test_Base {
	
	public 验证零部件RRR计算的查看标注记录功能是否可以实现_打开查询窗口_ID检索_4249(){
		super();
	}
	
	public 验证零部件RRR计算的查看标注记录功能是否可以实现_打开查询窗口_ID检索_4249(WebDriver driver){
		super(driver);
	}
	...
	public void detailedOperation(){
		...
		WebDriverConfiguration.operationInterval();
		testId("CA","","");
		testId("CA","2016-03-18","");
		testId("CA","2016-03-18", "2016-05-28");
		testId("CA_5_13182570 / 1.0","2016-03-18", "2016-05-28");
		testId("CA_5_13182570","2016-03-18", "2016-05-28");
		/*
	...
	public void testId(String Id,String startTime, String endTime){
		...
		getElement(ElementEnum_零部件RRR计算_345.ID查询).clear();
		getElement(ElementEnum_零部件RRR计算_345.ID查询).sendKeys(Id);
			...
			if(startTime.compareTo(endTime) > 0){
				...
				// 检查查询结果中是否包含所查关键字
					for(int i = 1;i < eles.size();i++){
						...
						String chXpath = ElementEnum_零部件RRR计算_345.检索结果第一行ID名称.getName().replaceAll("tr\\[2\\]", "tr["+(i+1)+"]");
						String chProduceName = driver.findElement(By.xpath(chXpath)).getText();
						assertTrue(chProduceName.contains(Id)||createTime.compareTo(startTime) > 0||createTime.compareTo(endTime) < 0,"查询结果不符合要求");
					}
				}
			}
		}
