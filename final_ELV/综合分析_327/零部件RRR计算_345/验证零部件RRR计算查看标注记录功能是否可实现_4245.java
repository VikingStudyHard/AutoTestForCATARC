 ...
 * 所属模块: 零部件RRR计算(#345)
 * 用例名称: 验证零部件RRR计算查看标注记录功能是否可实现
 * 前置条件: 验证零部件RRR计算查看功能是否可实现
 * 测试步骤->1. 点击零部件RRR计算页面中的查看标注记录按钮
 * 测试预期->1. 页面出现提示信息：请选择一个MDS！
 * 测试步骤->2. 勾选两项或多项MDS信息，点击零部件RRR计算页面中的查看标注记录按钮
 * 测试预期->2. 页面出现提示信息：请只选择一个MDS！
 * 测试步骤->3. 勾选一项MDS信息，点击零部件RRR计算页面中的查看标注记录按钮
 * 测试预期->3. 跳转至新页面，页面为查看的数据标注记录，可查看所选信息的数据标注记录
 * 
 */

public class 验证零部件RRR计算查看标注记录功能是否可实现_4245 extends Selenium_Test_Base {
	
	public 验证零部件RRR计算查看标注记录功能是否可实现_4245(){
		super();
	}
	
	public 验证零部件RRR计算查看标注记录功能是否可实现_4245(WebDriver driver){
		super(driver);
	}
	...
	public void detailedOperation(){
		//choose no message
				getElement(ElementEnum_零部件RRR计算_345.MSD信息查看标注记录按钮).click();
				WebDriverConfiguration.operationInterval();
