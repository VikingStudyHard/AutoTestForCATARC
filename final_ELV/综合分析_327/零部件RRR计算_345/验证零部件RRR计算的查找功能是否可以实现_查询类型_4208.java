 ...
 * 所属模块: 零部件RRR计算(#345)
 * 用例名称: 验证零部件RRR计算的查找功能是否可以实现-查询类型
 * 前置条件: 能正确进入综合分析模块
...
public class 验证零部件RRR计算的查找功能是否可以实现_查询类型_4208 extends Selenium_Test_Base {
	
	public 验证零部件RRR计算的查找功能是否可以实现_查询类型_4208(){
		super();
	}
	
	public 验证零部件RRR计算的查找功能是否可以实现_查询类型_4208(WebDriver driver){
		super(driver);
	}
