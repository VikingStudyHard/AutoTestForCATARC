 ...
 * 所属模块: 零部件RRR计算(#345)
 * 用例名称: 验证零部件RRR计算的查看标注记录功能是否可以实现-打开查询窗口-产品名称检索
 * 前置条件: 能正确进入综合分析模块
 * 测试步骤->1. 勾选一项MDS信息，点击零部件RRR计算页面中的查看标注记录按钮
 * 测试预期->1. 跳转至新页面，页面为查看的数据标注记录，可查看所选信息的数据标注记录
 * 测试步骤->2. 点击新页面右上角的查询窗口按钮
 * 测试预期->2. 出现查询窗口
 * 测试步骤->3. 在零部件RRR计算的查询条件：名称中填写1，点击检索按钮
 * 测试预期->3. 出现提示信息：请输入开始时间
 * 测试步骤->4. 选择开始时间为2016年3月18日，点击检索按钮
 * 测试预期->4. 出现提示信息：请输入结束时间
 * 测试步骤->5. 选择结束时间为2016年5月28日，点击检索按钮
 * 测试预期->5. 产品名称中含有字段1的所有MDS被检索出来
 * 测试步骤->6. 在零部件RRR计算的查询条件：名称中填写ca，点击检索按钮
 * 测试预期->6. 产品名称中含有字段ca的所有MDS被检索出来
 * 测试步骤->7. 在零部件RRR计算的查询条件：名称中填写卡达克，点击检索按钮
 * 测试预期->7. 产品名称中含有字段卡达克的所有MDS被检索出来
 * 
 */

public class 验证零部件RRR计算的查看标注记录功能是否可以实现_打开查询窗口_产品名称检索_4247 extends Selenium_Test_Base {
	
	public 验证零部件RRR计算的查看标注记录功能是否可以实现_打开查询窗口_产品名称检索_4247(){
		super();
	}
	
	public 验证零部件RRR计算的查看标注记录功能是否可以实现_打开查询窗口_产品名称检索_4247(WebDriver driver){
		super(driver);
	}
	...
	public void detailedOperation(){
		...
		WebDriverConfiguration.operationInterval();
		testProductName("1","","");
		testProductName("1","2016-03-18","");
		testProductName("1","2016-03-18", "2016-05-28");
		testProductName("ca","2016-03-18", "2016-05-28");
		/*
	...
	public void testProductName(String productName,String startTime, String endTime){
		...
		getElement(ElementEnum_零部件RRR计算_345.产品名称查询).clear();
		getElement(ElementEnum_零部件RRR计算_345.产品名称查询).sendKeys(productName);
			...
			if(startTime.compareTo(endTime) > 0){
				...
				// 检查查询结果中是否包含所查关键字
					for(int i = 1;i < eles.size();i++){
						...
						String chXpath = ElementEnum_零部件RRR计算_345.检索结果第一行产品名称.getName().replaceAll("tr\\[2\\]", "tr["+(i+1)+"]");
						String chProduceName = driver.findElement(By.xpath(chXpath)).getText();
						assertTrue(chProduceName.contains(productName)||createTime.compareTo(startTime) > 0||createTime.compareTo(endTime) < 0,"查询结果不符合要求");
					}
				}
			}
		}
