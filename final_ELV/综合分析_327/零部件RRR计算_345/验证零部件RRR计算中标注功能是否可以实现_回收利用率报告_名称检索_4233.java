 ...
 * 所属模块: 零部件RRR计算(#345)
 * 用例名称: 验证零部件RRR计算中标注功能是否可以实现-回收利用率报告-名称检索
 * 前置条件: 能正确进入综合分析模块
...
public class 验证零部件RRR计算中标注功能是否可以实现_回收利用率报告_名称检索_4233 extends Selenium_Test_Base {
	
	public 验证零部件RRR计算中标注功能是否可以实现_回收利用率报告_名称检索_4233(){
		super();
	}
	
	public 验证零部件RRR计算中标注功能是否可以实现_回收利用率报告_名称检索_4233(WebDriver driver){
		super(driver);
	}
