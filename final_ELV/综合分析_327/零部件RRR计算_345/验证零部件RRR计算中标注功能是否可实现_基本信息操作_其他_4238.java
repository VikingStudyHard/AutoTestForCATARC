 ...
 * 所属模块: 零部件RRR计算(#345)
 * 用例名称: 验证零部件RRR计算中标注功能是否可实现-基本信息操作-其他
 * 前置条件: 能正常进入综合分析模块
 * 测试步骤->1. 单击左键选择一条MDS信息，点击标注按钮
 * 测试预期->1. 跳转至新页面，页面包含零部件RRR计算基础信息表格
 * 测试步骤->2. 点击基本信息界面右上角的返回按钮
 * 测试预期->2. 页面被正常关闭，返回至零部件RRR计算界面
 * 测试步骤->3. 单击左键选择一条车型信息，点击标注按钮
 * 测试预期->3. 跳转至新页面，页面包含零部件RRR计算基础信息表格
 * 测试步骤->4. 点击基本信息界面右上角的校验按钮
 * 测试预期->4. 跳转至新页面，含有所有校验信息
 * 测试步骤->5. 点击校验信息界面的查看按钮
 * 测试预期->5. 出现提示信息：请选择一条信息！
 * 测试步骤->6. 选择一条校验信息，点击查看按钮
 * 测试预期->6. 页面跳转回零部件RRR计算基础信息表格，所选择的校验信息详细显示在节点信息模块中
 * 
 */

public class 验证零部件RRR计算中标注功能是否可实现_基本信息操作_其他_4238 extends Selenium_Test_Base {
	
	public 验证零部件RRR计算中标注功能是否可实现_基本信息操作_其他_4238(){
		super();
	}
	
	public 验证零部件RRR计算中标注功能是否可实现_基本信息操作_其他_4238(WebDriver driver){
		super(driver);
	}
