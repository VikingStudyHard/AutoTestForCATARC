 ...
 * 所属模块: 行业动态(#355)
 * 用例名称: 验证行业动态下方工具栏是否可实现-含异常操作
 * 前置条件: 能正常进入法规信息模块
 * 测试步骤->1. 点击行业动态界面最下方的翻页功能栏中的下一页按钮
 * 测试预期->1. 页面信息跳转至下一页，页面其余内容正常显示。
 * 测试步骤->2. 点击行业动态界面最下方的翻页功能栏中的最后一页按钮
 * 测试预期->2. 页面信息跳转至最后一页，页面其余内容正常显示。
 * 测试步骤->3. 点击行业动态界面最下方的翻页功能栏中的填写框填写1，按回车键。
 * 测试预期->3. 页面信息跳转至第1页，页面其余内容正常显示。
 * 测试步骤->4. 点击行业动态界面最下方的翻页功能栏中的填写框填写2000000，按回车键。
 * 测试预期->4. 页面信息跳转至最后一页，页面其余内容正常显示。
 * 
 */

public class 验证行业动态下方工具栏是否可实现_含异常操作_4475 extends Selenium_Test_Base {
	
	public 验证行业动态下方工具栏是否可实现_含异常操作_4475(){
		super();
	}
	
	public 验证行业动态下方工具栏是否可实现_含异常操作_4475(WebDriver driver){
		super(driver);
	}
