 ...
 * 测试预期->3. 跳转至新页面，页面为所选择查看标准信息的详细内容，且内容不可更改。
 * 测试步骤->4. 点击查看页面上的关闭按钮
 * 测试预期->4. 页面被正常关闭，返回到最初的标准信息界面。
 * 
 */

public class 验证标准信息查看功能是否可实现_4531 extends Selenium_Test_Base {
	
	public 验证标准信息查看功能是否可实现_4531(){
		super();
	}
	
	public 验证标准信息查看功能是否可实现_4531(WebDriver driver){
		super(driver);
	}
	...
	public void detailedOperation(){
		...
		getElement(ElementEnum_标准信息_352.查看按钮).click();
		WebDriverConfiguration.operationInterval();
		a.verifyTipsAndThenOperateButton("请选择信息", ElementEnum_标准信息_352.第一页提示信息, ElementEnum_标准信息_352.第一页提示框确认按钮);
		...
		driver.findElement(By.xpath(ElementEnum_标准信息_352.检索结果第一行勾选框.getName().replaceAll("tr\\[2\\]", "tr[3]"))).click();
		WebDriverConfiguration.operationInterval();
		assertNotEquals(getElement(ElementEnum_标准信息_352.查看按钮).getCssValue("display"), "none");
		assertNotEquals(getElement(ElementEnum_标准信息_352.修改按钮).getCssValue("display"), "none");
		getElement(ElementEnum_标准信息_352.查看按钮).click();
		WebDriverConfiguration.operationInterval();
