 ...
 * 所属模块: 最新法规(#354)
 * 用例名称: 验证最新法规新增功能是否可实现-编辑文本3
 * 前置条件: 能正常进入法规信息模块
 * 测试步骤->1. 点击页面中的新增按钮
 * 测试预期->1. 跳转至新页面，页面为新增最新法规信息的详细内容界面，可填写标题等详细文本内容。
 * 测试步骤->2. 填写标题和类别为：test，输入原文链接的地址，如www.baidu.com，点击保存
 * 测试预期->2. 显示更新法规信息成功
 * 测试步骤->3. 点击确定
 * 测试预期->3. 页面返回至最新法规
...
public class 验证最新法规新增功能是否可实现_编辑文本3_6834 extends Selenium_Test_Base {
	
	public 验证最新法规新增功能是否可实现_编辑文本3_6834(){
		super();
	}
	
	public 验证最新法规新增功能是否可实现_编辑文本3_6834(WebDriver driver){
		super(driver);
	}
